...
for (const item of simpsons) {
    ...
    surnameDiv.innerText = `${item.surname}`;
...
for (const item of coursesArray) {
    ...
    divElement.appendChild(modules);
...
divElement.style.fontWeight = '40px';
...
let ulElement = document.getElementsByClassName('menu');
...
for (const value of coursesAndDurationArray) {
    let divElement = document.createElement('div');
    divElement.innerText = `${value.title} ${value.monthDuration}`
    document.body.appendChild(divElement);
}
